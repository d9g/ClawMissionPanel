...
const blockedTasksRouter = require('./blocked-tasks-api');
...
// 阻塞任务 API
app.use('/api/tasks/blocked', blockedTasksRouter);
...
app.get('/api/tasks/:taskId/dependencies', async (req, res) => {
  try {
    ...
    const taskDeps = dependencyParser.parseTaskDependencies(`${taskId}.md`);
    const canStart = dependencyParser.canTaskStart(taskId);
    const chain = dependencyParser.getDependencyChain(taskId);
    ...
    res.json({
      ...
      data: {
        ...
        dependents: taskDeps.dependents,
        canStart: canStart.canStart,
        blockingTasks: canStart.blockingTasks,
        dependencyChain: chain,
      },
    });
  } catch (error) {
    console.error('依赖关系 API 错误:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

app.get('/api/tasks/dependency-graph', async (req, res) => {
  try {
    const dependencyParser = require('./dependency-parser');
    
    const graph = dependencyParser.buildDependencyGraph();
    const circular = dependencyParser.detectCircularDependencies();
    ...
    res.json({
      ...
      data: {
        nodes: graph.nodes,
        edges: graph.edges,
        hasCircular: circular.hasCircular,
        cycles: circular.cycles,
      },
    });
  } catch (error) {
    console.error('依赖图 API 错误:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

app.get('/api/tasks/dependency-blocks', async (req, res) => {
  try {
    const blockDetector = require('./dependency-block-detector');
    
    const blockResult = blockDetector.checkDependencyBlocks();
    ...
    console.error('依赖阻塞 API 错误:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
app.get('/api/reviews', async (req, res) => {
  try {
    const reviews = reviewApi.getAllReviews();
    res.json({ success: true, data: reviews });
  } catch (error) {
    console.error('评审任务 API 错误:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
app.get('/api/reviews/pending', async (req, res) => {
  try {
    const reviews = reviewApi.getPendingReviews();
    res.json({ success: true, data: reviews });
  } catch (error) {
    console.error('待评审任务 API 错误:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
app.get('/api/reviews/:reviewId', async (req, res) => {
  try {
    const review = reviewApi.getReviewById(req.params.reviewId);
    if (!review) {
      res.status(404).json({ success: false, error: '评审不存在' });
    } else {
      res.json({ success: true, data: review });
    }
  } catch (error) {
    console.error('获取评审详情错误:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
